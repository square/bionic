...
class ValidatingAndSavingToDiskProvider(BaseDerivedProvider):
    ...
    def compute_values_from_deps(self, dep_values):
        ...
        value_hash = self._generate_value_hash(value, value_path)
        return Artifact(
    ...
    def _generate_value_hash(self, value, value_path):
        protocol = self.protocol
        value_hash = protocol.tokenize_file(value_path)

        FIXME_code_versioning_policy = FIXME self.func_attrs.code_versioning_policy
        code_version = FIXME_code_versioning_policy.version
        if not code_version.includes_bytecode:
            return ""

        protocol = self.protocol
        # FIXME wait... shouldn't we be looking at versioning_policy.treat_bytecode_as_functional?
        versioning_policy = FIXME_context.core.versioning_policy
        try:
            extra_value_hash = protocol.get_extra_value_hash(
                value,
                FIXME_code_versioning_policy.suppress_bytecode_warnings
                or versioning_policy.ignore_bytecode_exceptions,
            )
        except Exception:
            if not versioning_policy.ignore_bytecode_exceptions:
